        ...
        <v-form ref="form" v-model="valid" lazy-validation>
          ...
          <v-flex class="d-flex justify-end pt-4">
            <v-alert
              ...
              shaped
              :type="tipoAlerta"
              v-model="apareceMensajeGuardado"
              >{{ mensajeGuardado }}</v-alert
            >
          </v-flex>
        </v-form>
      </v-col>
...
export default {
  ...
  async mounted() {
    ...
    this.fechaNacimiento = formatearFechaInput(
  ...
  methods: {
    ...
    async guardarDatos() {
      if (this.$refs.form.validate()) {
        let auxConsi = [];
        let auxConsiAntigua = [];
        //Sistema de guardado para las consideraciones médicas
        if (
          this.consideracionesMedicas &&
        ...
        const nuevosDatos = {
          ...
          consideraciones:
            JSON.stringify(auxConsi) === JSON.stringify(auxConsiAntigua)
              ? []
              : this.consideracionesMedicas,
        };
